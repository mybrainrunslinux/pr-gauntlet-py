...
- In `app/executor.py`, change session.flush() to session.commit() in the final workflow status update block. Verify with: `python -m pytest scoring/test_issues.py::test_20_chain_schedule -v` (must fail before fix, pass after).
